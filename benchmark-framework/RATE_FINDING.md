...
| `rampMaxDiscoveryMinutes`  | 75                                     | CHOP only — coupled to `rampHoldSeconds`: budget roughly `settle + 15 x rampHoldSeconds`, plus the drain if set. Hitting the cap reports the best rate that held without confirming it, with a WARN and no `rampVerification`                                                                                                                                                                                                                                                                                  |
